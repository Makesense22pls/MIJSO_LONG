...
/*   Updated: 2024/06/18 17:13:35 by mafourni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_len_char(t_all_the_time *evry)
{
	t_go	lib;
	...
	{
		evry->balade.nb_colum = ft_strlensolong(evry->map[lib.ycheh]);
		if (lib.ycheh > 0)
		{
			if (ft_strlensolong(evry->map[lib.ycheh - 1]) != evry->balade.nb_colum)
...
int	checking(t_all_the_time *all, char *str, t_stp *pls)
{
	if (!(full_map(str, all)))
		return (EXIT_FAILURE);
	if (!(check_len_char(all)) || !(check_map_place(all)))
		return (EXIT_FAILURE);
	need_pos(all, pls);
